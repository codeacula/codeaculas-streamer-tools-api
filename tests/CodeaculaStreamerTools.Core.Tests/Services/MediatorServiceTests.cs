namespace CodeaculaStreamerTools.Core.Tests.Services;
...
public class MediatorServiceTests
{
  ...
  private sealed class NullCommandHandler : ICommandHandler<NullCommand, string>
  {
    ...
    public Task<OperationResult<bool>> ValidateAsync(NullCommand command) => Task.FromResult(OperationResult.Ok(value: true));
  }
  ...
  private sealed class TestCommandHandler : ICommandHandler<TestCommand, string>
  {
    ...
    public Task<OperationResult<bool>> ValidateAsync(TestCommand command) => Task.FromResult(OperationResult.Ok(value: true));
  }
